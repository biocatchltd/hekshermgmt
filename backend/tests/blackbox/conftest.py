...
class HeksherService(SingleContainerService):
    def __init__(self, docker_client: DockerClient, *, db_connection_string: str, context_features: str, **kwargs):
        self.context_features = context_features
        self.db_connection_string = db_connection_string
        super().__init__(create_and_pull(docker_client, "biocatchltd/heksher:0.2.0", detach=True, publish_all_ports=True,
        environment={'HEKSHER_DB_CONNECTION_STRING': db_connection_string,
                     'HEKSHER_STARTUP_CONTEXT_FEATURES': context_features}))

    def start(self, retry_spec: Optional[RetrySpec] = None, **kwargs):
        super().start(**kwargs)
        def health_check(client: httpx.Client):
            client.get(f'http://localhost:{self.client_port()}/api/health').raise_for_status()
...
def heksher_service(sql_service, docker_client):
    with temp_network(docker_client) as network, connect(network, sql_service) as sql_alias:
        service = HeksherService(docker_client, db_connection_string=sql_service.container_connection_string(sql_alias[0]), context_features="user;trust;theme")
...
def app_client(monkeypatch, heksher_service):
    monkeypatch.setenv('HEKSHERMGMT_HEKSHER_URL', f'http://localhost:{heksher_service.client_port()}/')
