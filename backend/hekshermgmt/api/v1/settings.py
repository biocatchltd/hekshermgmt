...
class Rule(BaseModel):
    ...
    context_features: Dict[str, str] = Field(description="Context features for matching rule.")
    rule_id: int
    added_by: Optional[str] = Field(None, description="Creator of the rule.")
    information: Optional[str] = Field(None, description="Information on this rule, what this tries to achieve.")
...
async def get_setting_rules(setting_name: str, app: HeksherManagement = application):
    ...
    return [Rule(value=orjson.dumps(rule["value"]) if isinstance(rule["value"], dict) else rule["value"],
